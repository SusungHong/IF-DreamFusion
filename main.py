...
if __name__ == '__main__':
    ...
    if opt.image is not None:
        ...
        if opt.text is None:
            # use zero123 guidancemodel when only providing image
    ...
    if opt.test:
        ...
        if opt.guidance == 'deepfloyd-if':
            from guidance.if_utils import DeepFloyd_IF
            guidance= DeepFloyd_IF(device, opt.fp16, opt.vram_O, opt.if_version, opt.hf_key, opt.t_range)
        elif opt.guidance == 'stable-diffusion':
